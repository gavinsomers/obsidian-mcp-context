---
type: project
status: delayed
company: "[[Acme Corp]]"
source_created_at: 2026-05-22T10:31:00
source_observed_at: 2026-05-22T13:31:00
created_at: 2026-05-23T08:31:00
updated_at: 2026-06-11T09:31:00
tags: [#delivery]
---
# Project Pipeline

## Overview
Project Pipeline supports [[Acme Corp]]. Data warehouse migration with contract-delay reschedule and task carryover.

## Stakeholders
- [[David Chen]]
- [[Lina Ortega]]
- [[Hannah Brooks]]

## Risks
- [[Contract Signature Delay]]
- [[Warehouse Mapping Drift]]
- [[Analyst Enablement Gap]]

## Decisions
- [[Pipeline Kickoff Date Reset]]
- [[Pipeline Migration Wave Plan]]
- [[Acme Analyst Enablement Plan]]

## Open Loops
- [ ] Reconcile latest state for [[Acme Corp]] before next project review #ops
